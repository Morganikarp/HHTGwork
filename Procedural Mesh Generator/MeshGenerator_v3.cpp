...
void AMeshGenerator::ProcHexMeshCreate()
{
	...
	MaxNumberOfVerts = FMath::Pow( static_cast<double>(2), HexComplexity ) + 1;
	MinNumberOfVerts = FMath::Pow( static_cast<double>(2), HexComplexity - 1) + 1;
	...
	VertLayers = MaxNumberOfVerts - MinNumberOfVerts;
	...
	APlayerCharacter* const PlayerCharacter = Cast<APlayerCharacter>(UGameplayStatics::GetPlayerCharacter(this, 0));
	if (!IsValid(PlayerCharacter))
	{
		return;
	}

	// Sets the size of the high and low clamp arrays to equal the number of vertices in the mesh
	PlayerCharacter->ToolsManagerComponent->HighClamp.SetNum(Vertices.Num());
	PlayerCharacter->ToolsManagerComponent->LowClamp.SetNum(Vertices.Num());

	// Sets the default values of the high and low clamps
	for (int i = 0; i < Vertices.Num(); i++)
	{
		PlayerCharacter->ToolsManagerComponent->HighClamp[i] = PlayerCharacter->ToolsManagerComponent->TerrainCeilingClamp;
		PlayerCharacter->ToolsManagerComponent->LowClamp[i] = 0.0f;
	}
}
